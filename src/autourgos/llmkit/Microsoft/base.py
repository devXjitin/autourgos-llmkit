...
def azure_llm(
    ...
    *,
    ...
    # Detect if this is a MaaS endpoint (models.ai.azure.com) or Azure OpenAI (openai.azure.com)
    is_maas = "models.ai.azure.com" in azure_endpoint
    ...
    for attempt in range(1, max_retries + 1):
        try:
            if is_maas:
                # Use standard OpenAI client for MaaS (Serverless) endpoints
                # Ensure base_url ends with /v1 if not present, as required by some MaaS endpoints with OpenAI SDK
                ...
                # For MaaS, model name is often required but sometimes ignored if endpoint is model-specific.
                # We pass deployment_name as model.
                ...
                # Use AzureOpenAI client for standard Azure OpenAI Service
                client = AzureOpenAI(
                    api_key=api_key,
...
def azure_llm_stream(
    ...
    *,
    ...
    try:
        if is_maas:
             # Use standard OpenAI client for MaaS
            base_url = azure_endpoint
            if not base_url.endswith("/v1") and not base_url.endswith("/v1/"):
                    base_url = f"{base_url.rstrip('/')}/v1"
